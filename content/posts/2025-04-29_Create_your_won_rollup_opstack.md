+++
title = "Create your own L2 rollup testnet with the OP Stack"
...
In the OP Stack, the execution client is a fork of Geth called op-geth. Think of this as the "EVM Brain" of your L2 chain.

#### Consensus Client → op-node
The consensus client is responsible for:

- Tracking L1 blocks (on Ethereum Sepolia in our case)
- Coordinating with the execution client using the Engine API
- Determining the canonical L2 chain based on inputs from L1

This is handled by op-node — the core driver of your rollup. It watches Ethereum L1, processes L2 inputs, and drives op-geth to produce L2 blocks.

#### Batcher → op-batcher
Once your rollup is producing blocks, you need to post those blocks to Ethereum L1. The batcher:
...
- Submitting state roots of your L2 chain to L1
...
Create a `.envrc` file inside your optimism folder, and paste the exported environment variables into it, like this:
...
Make sure to replace `your-api-key` with your real Alchemy, Infura, or QuickNode endpoint for Sepolia.
...
`op-deployer` is a command-line tool built by the Optimism team to make deploying OP Stack contracts simple and repeatable. It handles:
...
You can open `.deployer/intent.toml` to tweak settings like:
...
- Deploy contracts like the L1StandardBridge, SystemConfig, and OutputOracle to Sepolia
- Record all deployed contract addresses into `.deployer/state.json`
- Follow the deployment flow you configured earlier

If everything works, you'll see success messages with deployed addresses.

If something fails (like "out of gas" errors), check:

- Fund balances on the Admin account
...
The `genesis.json` file defines the initial state for your Execution Client. It tells op-geth:
...
The `rollup.json` file defines the rollup configuration for your Consensus Client. It tells `op-node`:
...
These two clients will work together to drive your L2 rollup. Let's go step-by-step.
...
Now that your Execution Client (op-geth) and Consensus Client (op-node) are live and producing L2 blocks, you still need two critical background services to complete the full rollup lifecycle:

- `op-batcher` → Posts transaction data to L1
- `op-proposer` → Posts state roots to L1 (for withdrawals and finality)

#### Start the Batcher (op-batcher)
...
Now your batcher is running! It will:

- Monitor your op-geth
- Submit batched L2 transactions to Sepolia (L1)

#### Start the Proposer (op-proposer)
The proposer submits L2 state roots to the L2OutputOracle contract on Sepolia. This enables users to withdraw funds back to L1 eventually.

Open another terminal window and navigate to the proposer directory:
...
Now your proposer is live! It will:
...
Now that your rollup is fully running — producing blocks, posting batches, and finalizing states — it's time to connect your wallet (like MetaMask) and interact with your L2 network directly.
...
Open MetaMask and add a new custom network manually. Here's the information you'll need:
...
Save the network. Your MetaMask should now point to your local Rollup at localhost:8545.
...
To send transactions or deploy contracts, you need ETH on your L2 chain. How do you get ETH onto your rollup? By bridging it through the L1 Standard Bridge contract you deployed earlier.

Let's do it step-by-step.

#### Find the L1 Bridge Contract Address
...
This will output the address you need to send ETH to. Save this address.
...
You can now send transactions, deploy smart contracts, and observe your rollup in action. Let's walk through the basic tests you should do.
...
- The transaction confirmed in your wallet.
...
For a full test, let's also deploy a simple smart contract. You can use tools like Remix IDE, Hardhat, or Foundry.
...
Compile and deploy it. ✅ You should see:
